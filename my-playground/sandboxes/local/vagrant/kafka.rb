...
$script_ubuntu_install_cmak = <<-'SCRIPT'
echo "###############################################################################"
...
cd CMAK
sudo sed -i 's/ -XX:+UseJVMCICompiler//g' sbt # Java 11.0.4+ does not support -XX:+UseJVMCICompiler
sudo ./sbt -java-home /usr/lib/jvm/java-11-openjdk-amd64 clean dist
cd target/universal
sudo unzip cmak-.*.zip
